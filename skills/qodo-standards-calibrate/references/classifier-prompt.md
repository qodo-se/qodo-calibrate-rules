...
The orchestrator spawns one subagent per group of 4–5 batches with this prompt, placeholders
...
Fill `<skill-dir>`, `<run-dir>`, and `<batches>` (e.g. `6 7 8 9 10`). Nothing else changes.
...
2. For every rule decide, from its **content** together with its name and category:
   - `tag`: exactly one taxonomy tag. When two fit, choose the one with the higher default
     severity ("never log tokens" is `secrets-handling`, not `logging`; "validate request bodies
     against the schema" is `security-control`, not `api-contract`).
   - `summary`: one sentence saying what the rule requires — at most 160 characters, on one line,
     with no ` · `, no `→`, and no `…` or `...`. Write it from the content; never paste a truncated
     slice of it. It is display-only and is shown to the admin beside the rule's name.
3. Write `<run-dir>/batches/batch-NNN.decisions.json`:
   `{"<ruleId>": {"tag": "<tag>", "summary": "<summary>"}, ...}` with an entry for **every** rule
...
4. Record it — one command:
   ...
   names each problem (a missing rule, an id not in the batch, an unknown tag, a summary that
   breaks the contract): fix the decisions file and run the same command again. A status of
...
one-line note for any batch you could not record and why. No rule text, no summaries, no
commentary.
