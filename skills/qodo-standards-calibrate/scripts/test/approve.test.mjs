...
function lineOf(ctx, ruleId) {
  return proposalRows(readText(ctx.proposal)).find((r) => r.text.startsWith(`- [x] ${ruleId} ·`) || r.text.startsWith(`- [ ] ${ruleId} ·`)).line;
}

test('an unedited proposal reads back as all approve plus the needs-a-decision skips', () => {
  const ctx = proposed();
  const rb = readback(ctx);
  assert.deepEqual(rb.counts, { approve: 5, skip: 2, override: 0, invalid: 0, removed: 0 });
  assert.equal(rb.counts.approve, 5); // frontmatter proposed
  assert.equal(rb.counts.skip, CALIB_DECISIONS.length);
  assert.deepEqual(rb.invalid, []);
  assert.equal(rb.readback_text, '5 approve · 2 skip · 0 override · 0 invalid override');
  assert.deepEqual(rb.rows.map((r) => r.rule_id), [99, 101, 102, 103, 104, 105, 106]);
  for (const row of rb.rows) {
    assert.equal(row.decision, CALIB_DECISIONS.includes(row.rule_id) ? 'skip' : 'approve');
...
test('unchecking, editing a target, and checking a needs-a-decision row read back as skip, override, approve', () => {
  ...
  const rb = readback(ctx);
  assert.deepEqual(rb.counts, { approve: 4, skip: 2, override: 1, invalid: 0, removed: 0 });
  ...
  assert.equal(by[106].decision, 'skip');
  assert.equal(rb.readback_text, '4 approve · 2 skip · 1 override · 0 invalid override');
});
...
test('an invalid override is listed by line with its reason and excluded', () => {
  ...
  assert.equal(rb.readback_text, `3 approve · 2 skip · 0 override · 2 invalid override (rows ${line103}: "critical" is not a severity; rows ${line104}: "warning" equals current severity)`);
...
test('a mangled row is invalid by line number and leaves the other rows alone', () => {
  ...
  assert.equal(rb.readback_text, `5 approve · 1 skip · 0 override · 1 invalid override (rows ${line}: unparseable row)`);
});
...
test('--record-skips appends the skipped rows and holds them on the next render', () => {
  ...
  assert.equal(first.json.recorded, 3);
  assert.deepEqual(first.json.rule_ids, [99, 105, 106]);
  ...
  assert.equal(ledgerLines(ctx.ledger).length, 3);

  // a readback after recording is unchanged: the rows are still in the file
  const rb = readback(ctx);
  assert.equal(rb.counts.skip, 3);
  ...
  assert.equal(rendered.json.rows, 4);
  const text = readText(join(nextRun, 'proposal.md'));
  for (const id of [99, 105, 106]) assert.ok(!text.includes(` ${id} · `), `rule ${id} must be held`);
...
test('recording again after the admin unchecks another row appends only that row', () => {
  ...
  assert.deepEqual(again.json.already_recorded, [99, 105, 106]);
  assert.equal(ledgerLines(ctx.ledger).length, 3);
  ...
  assert.deepEqual(third.json.already_recorded, [99, 105, 106]);
  assert.equal(third.json.counts.skip, 4);
  const lines = ledgerLines(ctx.ledger);
  assert.equal(lines.length, 4);
  assert.deepEqual(lines.map((l) => l.rule_id), [99, 105, 106, 102]);
});

test('a reconsidered rule can be skipped again in the same run and is held afterwards', () => {
  const ctx = proposed();
  edit(ctx, [['- [x] 99 ·', readText(ctx.proposal).split('\n').find((l) => l.startsWith('- [x] 99 ·')).replace('- [x]', '- [ ]')]]);
  assert.equal(run(APPROVE, ['--run', ctx.runDir, '--record-skips'], { env: ctx.env }).json.recorded, 3);
  ...
  assert.equal(rerender.json.held_by_prior_decision, 2);
  ...
  const lines = ledgerLines(ctx.ledger);
  assert.deepEqual(lines.map((l) => [l.rule_id, l.decision]), [[99, 'skip'], [105, 'skip'], [106, 'skip'], [99, 'released'], [99, 'skip']]);
  const final = run(PROPOSAL, ['--run', ctx.runDir, '--render', '--workspace-id', 'ws-1', '--replace'], { env: ctx.env });
  assert.equal(final.json.held_by_prior_decision, 3);
  assert.ok(!readText(ctx.proposal).includes(' 99 · '));
});
...
test('a rule at an unknown severity renders, reads back, and can be approved', () => {
  ...
  assert.deepEqual(before.counts, { approve: 1, skip: 1, override: 0, invalid: 0, removed: 0 });
  // checking it approves the rubric's severity for its tag
  edit(ctx, [['- [ ] 501 ·', text.split('\n').find((l) => l.startsWith('- [ ] 501 ·')).replace('- [ ]', '- [x]')]]);
  const rb = readback(ctx);
  assert.deepEqual(rb.counts, { approve: 2, skip: 0, override: 0, invalid: 0, removed: 0 });
